...
def init_zarr(model, region, all_track_ids): 
    ...
    models.init_donefile(model, region, tag=f'mcs_precip_efficiency').touch()
...
def compute_track_PE(ds, model, region): 
    ...
    # Build index mapping from actual track IDs in the zarr
    ...
    # Compute first timestep for each track from the zarr
    first_3h_step = np.full(n_tracks, -1, dtype=np.int32)
    for out_i, tid in enumerate(all_track_ids):
        timesteps = np.where((track_id_array == tid).any(axis=1))[0]
        if len(timesteps) > 0:
            first_3h_step[out_i] = timesteps[0]
